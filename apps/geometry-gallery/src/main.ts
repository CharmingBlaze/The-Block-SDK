import { createEditor, validateMesh, type LibraryGeometryId } from "@modeling-kit/sdk";
...
import { GALLERY_SHAPES, galleryLabel, paramsForLibraryShape } from "./shapes";
...
for (const kind of GALLERY_SHAPES) {
  const option = document.createElement("option");
  option.value = kind;
  option.textContent = galleryLabel(kind);
  shapeSelect.appendChild(option);
}

function show(kind: LibraryGeometryId): void {
  wire = applyPreview(editor, viewport, materials, { ...state, showWire: false }, wire);
  editor.clear();
  editor.spawn.library(kind, { ...paramsForLibraryShape(kind), name: galleryLabel(kind) });
...
shapeSelect.addEventListener("change", () => {
  index = Math.max(0, GALLERY_SHAPES.indexOf(shapeSelect.value as LibraryGeometryId));
  show(GALLERY_SHAPES[index]!);
});
document.getElementById("prev")?.addEventListener("click", () => {
  index = (index + GALLERY_SHAPES.length - 1) % GALLERY_SHAPES.length;
  shapeSelect.value = GALLERY_SHAPES[index]!;
  show(GALLERY_SHAPES[index]!);
});
