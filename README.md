...
- **Rigging & Animation (`@modeling-kit/rigging`, `@modeling-kit/animation`)**: Preview-quality directed bone graphs, inverse bind matrices, and linear blend skinning. Canonical clips live on `ModelDocument` (`position` / `constant` | `linear` | `cubic`). The older glTF-shaped sampler (`translation` / `CUBICSPLINE`) is not a second source of truth; unsupported spline sampling is rejected.
...
- **Asynchronous Workers (`@modeling-kit/workers`)**: `AsyncComputePool` currently covers triangulation, UV packing, and topology validation on the main-thread fallback. Dedicated paint/IO worker jobs are not implemented.
