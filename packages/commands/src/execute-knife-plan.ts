...
export interface ExecuteKnifePlanParams {
  readonly points: readonly Vec3Tuple[];
  readonly snapRadius?: number;
}

export class ExecuteKnifePlanCommand implements Command<KnifeExecuteResult> {
  ...
  constructor(readonly params: ExecuteKnifePlanParams) {}
  ...
  execute(context: CommandContext): KnifeExecuteResult {
    if (this.after && this.meshId) {
      ...
      if (!this.result) {
        throw new Error("ExecuteKnifePlanCommand is missing a stored result");
      }
      return this.result;
    }
