...
import type { AnimationClip, KeyframeTrack } from "./legacy-types";
...
export function validateKeyframeTrack(track: KeyframeTrack): void {
  ...
  if (track.interpolation === "CUBICSPLINE") {
    throw new RangeError(
      "CUBICSPLINE interpolation is not implemented; convert to document cubic keys or use LINEAR",
    );
  }
  ...
  if (track.values.length !== expected) {
    throw new RangeError(`Keyframe values length ${track.values.length} must equal ${expected}`);
  }
