...
import { runTriangulationBackend } from "./run-backend";
import type { PolygonTriangulation, PolygonTriangulationOptions, Vec2, Vec3 } from "./types";
import { polygonSelfIntersects, polygonSelfIntersects3d, requireFiniteLoops } from "./validate-input";
import { emptyTriangulation } from "./validate-output";
...
export function triangulatePolygonLoops(
  ...
  const result = runTriangulationBackend(
    backend,
    outerSource,
    outerProjected,
    alignedHoles,
  ...
  if (backend === "earcut" && requested !== "earcut" && alignedHoles.length === 0) {
    const fallback = runTriangulationBackend("earclip", outerSource, outerProjected, [], reversed, areaTolerance);
    if (fallback) {
