...
| ------- | ------- | ---- |
| `three-mesh-bvh` | optional peer of `three-adapter` as `SpatialQueryBackend` | Triangle/mesh BVH still optional. Object AABB `BvhSpatialQuery` plus first-party `MeshLocalBvh` (topology rebuild, position refit) ship without this package |
...
- `SpatialQueryBackend` — first-party `BvhSpatialQuery` (AABB, revision-aware) in the adapter; `MeshLocalBvh` in `@modeling-kit/mesh` for triangle queries; hosts may wrap `three-mesh-bvh`  
